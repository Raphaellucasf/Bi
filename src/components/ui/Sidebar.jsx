...
import { useCompromissosDiarios } from '../../hooks/useCompromissosDiarios';

const Sidebar = ({ isCollapsed = false, onToggle }) => {
  const location = useLocation();
  const [isMobileOpen, setIsMobileOpen] = useState(false);
  const compromissosPendentes = useCompromissosDiarios();
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      to={item.path}
                      onClick={closeMobileMenu}
                      className={`relative group flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${
                        isActive
                    ...
                    >
                      ...
                      {!isCollapsed && item.badge > 0 && (
                        <span className="absolute right-2 top-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                          {item.badge}
                        </span>
                      )}
                    </Link>
