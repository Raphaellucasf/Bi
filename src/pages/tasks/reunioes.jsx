...
const Reunioes = () => {
  ...
  const fetchProcessos = async (search = '') => {
    let query = supabase.from('processos').select('id, numero_processo, clientes:cliente_id(nome)');
    if (search) {
      query = query.ilike('numero_processo', `%${search}%`).or(`clientes.nome.ilike.%${search}%`);
    }
  ...
  const handleProcessoSearch = e => {
    setProcessoSearch(e.target.value);
    fetchProcessos(e.target.value);
  };
      ...
      <main className="transition-all duration-300 pt-16 ml-0 md:ml-60">
        <div className="p-8 max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="mb-8 space-y-4 p-4 bg-white rounded shadow">
            <h2 className="text-lg font-bold mb-2">Nova Reunião</h2>
            <div>
              ...
              <Select
                ...
                options={processos.map(p => ({ value: p.id, label: `${p.numero_processo} (Cliente: ${p.clientes?.nome || '-'})` }))}
          ...
          {andamentos.length === 0 ? (
            <div className="text-muted-foreground">Nenhuma reunião cadastrada.</div>
          ) : (
